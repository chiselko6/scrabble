...
Run the server:
    ...
    $ docker run --rm -it -p 5678:5678 scrabble:latest python run_cmd.py host --port 5678 --host 0.0.0.0

Run the client:

    $ docker build -t scrabble:latest .
    $ docker run --rm -it --network host scrabble:latest python run_cmd.py player <username> <game_id> <server_host> <server_port>
